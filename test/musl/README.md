# Go musl Compatibility Test Suite

This directory contains tests for verifying Go's compatibility with musl libc, particularly for c-shared and c-archive build modes.

## Background

Go makes assumptions about the C runtime that are specific to glibc but not guaranteed by standards:
1. DT_INIT_ARRAY functions receive (argc, argv, envp) - only glibc does this
2. Initial-exec TLS model for shared libraries - incompatible with dlopen on musl

## Test Structure

### Unit Tests
- `src/runtime/cgo_musl_test.go` - Go test that builds and loads a shared library
- `src/runtime/testdata/testprogcgo/musl_sharedlib.go` - Test shared library
- `src/runtime/testdata/testprogcgo/musl_loader.c` - C program that uses dlopen
